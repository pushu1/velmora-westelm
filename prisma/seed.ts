...
async function main() {
    ...
    data: {
      ...
      addresses: {
        create: [
          {
            name: 'Home Address',
            street: '123 Luxury Avenue, DLF Phase 5',
            city: 'Gurugram',
            state: 'Haryana',
            postalCode: '122002',
            country: 'India',
  ...
  console.log('👤 Created default users (Admin & Customer).');

  // 3. Create Categories
  const categoriesData = [
    { name: 'Furniture', slug: 'furniture', description: 'Modern sofas, dining tables, chairs, and bedroom sets designed for elegant living.' },
    { name: 'Bedding', slug: 'bedding', description: 'Premium duvet covers, sheets, quilts, and pillows crafted from the finest organic linen and cotton.' },
    { name: 'Bath', slug: 'bath', description: 'Luxurious towels, bath mats, shower curtains, and bath accessories to create a spa-like retreat.' },
    { name: 'Outdoor', slug: 'outdoor', description: 'Weather-resistant outdoor sofas, lounge chairs, dining tables, and concrete planters.' },
  ];
  ...
  console.log('📂 Created categories: Furniture, Bedding, Bath, Outdoor.');

  // 4. Seeding Products & Variants
  // We will seed 20 products with descriptive details and 2 variants each,
  // where each variant has a list of high-quality Unsplash image URLs (1st = main, 2nd = secondary/hover/lifestyle).
  ...
  console.log(`🎁 Ready to seed ${productsData.length} products with multiple variants...`);

  for (const prodInfo of productsData) {
    const categoryId = categoriesMap[prodInfo.categoryName];
    if (!categoryId) {
      console.warn(`⚠️ Category not found: ${prodInfo.categoryName} for product ${prodInfo.title}`);
      continue;
    }
    ...
    const createdProduct = await prisma.product.create({
      data: {
        ...
        basePrice: prodInfo.basePrice,
        discountPrice: prodInfo.discountPrice,
        inventoryCount: prodInfo.inventoryCount,
        categoryId: categoryId,
      },
    });

    for (const varInfo of prodInfo.variants) {
      await prisma.variant.create({
        data: {
          productId: createdProduct.id,
          colorName: varInfo.colorName,
          colorHex: varInfo.colorHex,
          sku: varInfo.sku,
          stock: varInfo.stock,
          imageUrls: varInfo.imageUrls,
        },
      });
    }
  }
